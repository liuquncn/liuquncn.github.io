<?
  session_start();
	...
	function replace_text_with_html($txt)
	{
...
//  echo $_SESSION['authnum']."s<br>";
//  echo $HTTP_SESSION_VARS[authnum]."<br>";

  if (isset($_POST["authinput"]))
  {
	  if ($_POST["authinput"]!=$_SESSION["authnum"] || empty($_SESSION["authnum"]))
	  {
		  echo "验证码不正确。<br>";
	...
	if( $banIP == 1 )
	{
		...
		{
			error( "Sorry, Our message board is currently being updated, please come back later" );
			exit();
		}
	}
	if( !isset( $action ) ) error( "Invaild page 'post.php' call" );
	$topic = replace_text_with_html(trim( $topic ));
	$name = replace_text_with_html(trim( $name ));
	$email = replace_text_with_html(trim( $email ));
	$msg = replace_text_with_html(trim( $msg ));
