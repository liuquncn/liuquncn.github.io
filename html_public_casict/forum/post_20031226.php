<?
	...
	$email = trim( $email );
	$msg = trim( $msg );
	if( $topic == "" ) error( "Topic required" );
	else $topic = textFilter( $topic );
	if( $topic == "" ) error( "Invalid topic" );
	if( $name == "" ) error( "Name required" );
	elseif( !eregi( "[a-z0-9].", $name ) ) error( "Invaild name" );
	...
	if( $msg == "" ) error( "Message content required" );
	else $msg = enableHTML( textFilter( $msg ) );
	if( $msg == "" ) error( "Invalid message content" );
